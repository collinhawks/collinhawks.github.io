  ...
  export default {
      data() {
          return {
              ...
              allParticles: [], // Array to hold all particles
          };
      },
      mounted() {
      ...
      methods: {
          ...
          createAllParticles() {
              for (let i = 0; i < this.particleSettings.count * 3; i++) {
                  this.allParticles.push(this.Particle());
              }
          },
          distributeParticles() {
              const numParticles = this.particleSettings.count;
              const totalParticles = numParticles * 3;
  
              this.nearParticles = this.allParticles.slice(0, numParticles);
              this.middleParticles = this.allParticles.slice(numParticles, numParticles * 2);
              this.farParticles = this.allParticles.slice(numParticles * 2, totalParticles);
          },
          createParticles() {
          ...
          startSnowfall() {
              ...
              // Populate the particles array with all particles
              this.particles = [...this.nearParticles, ...this.middleParticles, ...this.farParticles];
  
              // Sort the particles randomly
          ...
          createSnowfall(particles, flag) {
            while (particles.length < this.particleSettings.count) {
                ...
                if (Math.random() < 0.25) { // 25% chance to create new particle
                    ...
                    console.log("Particle reuseCount: ", this.reusedParticleCount);
